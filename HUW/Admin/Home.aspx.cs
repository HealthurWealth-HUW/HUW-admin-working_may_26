...
public partial class Admin_Home : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                lbldeletedProducts.Text = Convert.ToString(repository.Count(p=> p.IsDeleted == false));
                lblinactive.Text= Convert.ToString(repository.Count(p => p.IsActive == false));
                ...
                lblPickupOrders.Text = Convert.ToString(AuthorizedOrders.Count(p => p.TxnStatus == "SUCCESS" && p.Dispatched == false && p.Delivered == false && p.Pickup == true && p.Authorized == true&&(p.IsPacking==null||p.IsPacking==false)));
                lblinpacking.Text = Convert.ToString(AuthorizedOrders.Count(p => p.TxnStatus == "SUCCESS" && p.Dispatched == false && p.Delivered == false && p.Pickup == true && p.Authorized == true&&p.IsPacking==true));
                ...
                //lblrefundorders.Text = Convert.ToString(AuthorizedOrders.Count(p => p.OrderCurrentStatus == 5||p.Orderdeliverystatus.Contains("rto")));
    ...
    private void FillGrid()
    {
        ...
        List<Ouofstock> data = Entity.Database.SqlQuery<Ouofstock>("SELECT top(20)  OOS.*,P.ProductName, COALESCE(x.cnt,0) AS Notifymecount  FROM [dbo].[Tbl_Outofstock] AS OOS JOIN Products AS P ON P.ProductId = OOS.ProductId lEFT JOIN(select ProductId, CreatedOn, count(*) cnt from NoftifyMe Group by ProductId, CreatedOn) x on x.ProductId = oos.Productid and x.CreatedOn = oos.Soldoutdate  order by oos.Soldoutdate desc").ToList();
