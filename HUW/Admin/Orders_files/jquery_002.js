...
function SearchDynatree(SearchText, hdnVal, divDyna) {
    if (SearchText.value != '') {
        var nodeList = [];
        var x = SearchText.value;
        BuildTree(JSON.parse($("input[id$=" + hdnVal + "]").val()), divDyna);
        var test = $("div[id$=" + divDyna + "]").dynatree("getTree");
        var pNode = test.options.children[0].title;
        test.tnRoot.visit(function (node) {
            if (node.data.title.toLowerCase().indexOf(x.toLowerCase()) >= 0) {
                if (node.data.title.trim().toLowerCase() == pNode.toLowerCase()) {
                } else {
                    nodeList.push(node.toDict(true));
                }
            }
        });
    }
    SelectDynaCase(nodeList, SearchText, divDyna, hdnVal);
}

function BuildTree(data, divDyna) {
    var Root = $("div[id$=" + divDyna + "]").dynatree("getRoot");
    Root.removeChildren();
    Root.addChild(data);
}

function SelectDynaCase(data, SearchText, divDyna, hdnVal) {
    if (SearchText.value != '') {
        BuildTree(data, divDyna);
    }
    else {
        BuildTree(JSON.parse($("input[id$=" + hdnVal + "]").val()), divDyna);
    }
    $.map($("div[id$=" + divDyna + "]").dynatree("getTree").getSelectedNodes(), function (dtnode) {
