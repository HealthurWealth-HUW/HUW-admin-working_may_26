<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/AdminMaster.master" AutoEventWireup="true" CodeFile="PickupOrders.aspx.cs" Inherits="Admin_PickupOrders" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function DownloadShipRocketExcel() {
            var isallselected = "";
            var d = new Date();
            var checkedVals = $('.CheckPickUP:checkbox:checked').map(function () { return this.value; }).get();
            var str_array = String(checkedVals).split(',');
            for (var i = 0; i < str_array.length; i++) {
                // Trim the excess whitespace.
                str_array[i] = str_array[i].replace(/^\s*/, "").replace(/\s*$/, "");
                // Add additional code here, such as:
                if ($("#ddbox_" + str_array[i]).val() == "") {
                    isallselected = "false";
                }
                else {
                    isallselected = "true";
            ...
            if (isallselected == "true") {
                $.ajax({
                    contentType: "application/json; charset=utf-8",
                    url: '../api/Master/DownloadShipRocketExcel?OrderIDs=' + checkedVals,
                    type: 'Get',
                    dataType: 'json',
                    success: function (data) {
                        DownloadPDF();
                        ...
                        $("#divEcomExcel").table2excel({
                            ...
                            //fileExtension: ".xlsx",
                            fileext: ".xlsx",
                            filename: d.getDate() + "_HUW_ShipRocket" //do not include extension
                        });
                    }
                });
            }
                ...
                alert('Please select box types for selected items');
            }
        }
        function DownloadBluedartExcel() {
            var isallselected = "";
            var d = new Date();
            var checkedVals = $('.CheckPickUP:checkbox:checked').map(function () { return this.value; }).get();
            var str_array = String(checkedVals).split(',');
            ...
            if (isallselected == "true") {
                $.ajax({
                    contentType: "application/json; charset=utf-8",
                    url: '../api/Master/DownloadBluedartExcel?OrderIDs=' + checkedVals,
                    type: 'Get',
                    dataType: 'json',
                    success: function (data) {
                        //alert(data.Result);
...
function DownloadDTDCExcel() {
            ...
            if (isallselected == "true") {
                $.ajax({
                    contentType: "application/json; charset=utf-8",
                    url: '../api/Master/DownloadDTDCExcel?OrderIDs=' + checkedVals,
                    type: 'Get',
                    dataType: 'json',
                    success: function (data) {
                        //alert(data.Result);
    ...
    <div id="content_wrap">
        <div id="cp_placeholder">
            ...
            <div id="activity_stats">
                <h1>
                    <label id="ctl00_ctl00_Main_Main_lblPageTitle">Waiting for Pickup Orders</label>
                </h1>
            </div>
            <!--Quick Actions-->
